  ...
  static get template() {
    ...
    <div id="scrollers" desktop\$="[[_desktopMode]]" on-track="_track">
      <vaadin-infinite-scroller id="monthScroller" on-custom-scroll="_onMonthScroll" on-touchstart="_onMonthScrollTouchStart" buffer-size="3" active="[[initialPosition]]" part="months">
        <template>
          <vaadin-month-calendar i18n="[[i18n]]" can-be-fuzzy="[[canBeFuzzy]]" accuracy="{{accuracy}}" month="[[_dateAfterXMonths(index)]]" selected-date="{{selectedDate}}" focused-date="[[focusedDate]]" ignore-taps="[[_ignoreTaps]]" show-week-numbers="[[showWeekNumbers]]" min-date="[[minDate]]" max-date="[[maxDate]]" focused\$="[[_focused]]" part="month" theme\$="[[theme]]">
...
`;
  ...
  _onThisMonthTap() {
    ...
    if (Math.abs(this.$.monthScroller.position - this._differenceInMonths(startOfMonth, this._originDate)) < 0.001) {
      // Select today only if the month scroller is positioned approximately
      // at the beginning of the current month
      this.accuracy = 'month';
  ...
  _onThisYearTap() {
    ...
    if (Math.abs(this.$.monthScroller.position - this._differenceInMonths(startOfYear, this._originDate)) < 0.001) {
      // Select today only if the month scroller is positioned approximately
      // at the beginning of the current month
      this.accuracy = 'year';
      this.selectedDate = startOfYear;
      this._close();
    } else {
      this._scrollToCurrentMonth();
    }
  ...
  _isThisMonthAllowed(min, max) {
    const today = new Date();
    const start = new Date(0, 0);
    const end = new Date(0, 0);
