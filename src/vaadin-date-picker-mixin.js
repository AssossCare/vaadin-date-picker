...
export const DatePickerMixin = subclass => class VaadinDatePickerMixin extends mixinBehaviors(
  ...
  static get observers() {
    return [
      ...
      '_selectedDateChanged(_selectedDate, i18n.formatDate,accuracy)',
  ...
  _parseDate(str) {
    ...
    date.setMonth(this.accuracy !== 'year' ? parseInt(parts[2], 10) - 1 : 0);
    date.setDate(this.accuracy === 'day' ? parseInt(parts[3], 10) : 1);
    return date;
  }
  ...
  _formatISO(date, accuracy) {
    if (!(date instanceof Date) || accuracy === undefined) {
      return '';
    }
    ...
    const month = accuracy === 'year' ? '00' : pad(date.getMonth() + 1);
    const day = accuracy !== 'day' ? '00' : pad(date.getDate());
  ...
  _selectedDateChanged(selectedDate, formatDate, accuracy) {
    if (selectedDate === undefined || formatDate === undefined || accuracy === undefined) {
      return;
    }
    if (this.__userInputOccurred) {
      this.__dispatchChange = true;
    }
    // return XXXX xx/xxxx xx/xx/xxxx
    const inputValue = selectedDate && formatDate(DatePickerHelper._extractDateParts(selectedDate), accuracy);
    // return XXXX-00-00
    const value = this._formatISO(selectedDate, accuracy);
    if (value !== this.value) {
  ...
  _onOverlayClosed() {
    ...
    if (this.i18n.parseDate) {
      ...
      const parsedDate = dateObject &&
        this._parseDate(`${dateObject.year}-${dateObject.month}-${dateObject.day}`);
  ...
  checkValidity(value) {
    var inputValid = !value ||
      (this._selectedDate && value === this.i18n.formatDate(DatePickerHelper._extractDateParts(this._selectedDate), this.accuracy));
  ...
  _onKeydown(e) {
    ...
    switch (this._eventKey(e)) {
      ...
      case 'up':
        ...
        break;
      case 'enter': {
        const date = this._inputValue;
        this.accuracy = /^(0[\/-])?(0[\/-])?-?[0-9]{2,4}$/.test(this._inputValue) ? 'year' :
          /^(0[\/-])?([0-1]?[1-9][\/-])-?[0-9]{2,4}$/.test(this._inputValue) ? 'month' :
            /^(([0-3][1-9])|[1-9])[\/-]?(([0,1][1-9])|[1-9])[\/-]?-?([0-9]){2,4}$/.test(this._inputValue) ? 'day' : this.accuracy;
        const dateObject = this.i18n.parseDate(date);
        const parsedDate = dateObject &&
          this._parseDate(dateObject.year + '-' + dateObject.month + '-' + dateObject.day);

        if (this._overlayInitialized && this._isValidDate(parsedDate)) {
          this._selectedDate = parsedDate;
        }
        this.close();
        break;
  ...
  _userInputValueChanged(value) {
    if (this.opened && this._inputValue) {
      ...
      const parsedDate = dateObject &&
        this._parseDate(`${dateObject.year}-${parseInt(dateObject.month) + 1}-${dateObject.day}`);
